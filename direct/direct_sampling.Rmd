...
load(here('simulation_daily_high_noLTFU.RData'))
...
save <- estimates_200 %>% select(week, prev_list) %>% rename(prev = prev_list)
save$week <- save$week-8
write.csv(save, here("sim_200.csv"))
...
```{r message=FALSE, warning=FALSE}
rm(list=ls())
load(here('simulation_daily_med.RData'))
```
...
```{r message=FALSE, warning=FALSE}
rm(list=ls())
load(here('simulation_daily_low.RData'))
